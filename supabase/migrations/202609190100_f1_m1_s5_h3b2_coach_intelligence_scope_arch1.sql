...
-- H3B2 outcome + learning tenant cutover follows.
...
begin
  ...
  with latest_outcome as (
    ...
    select
      x.item,
      (x.item->>'organization_id')::uuid as organization_id,
      l.action_code,
    ...
    left join learning l
      on l.organization_id=(x.item->>'organization_id')::uuid
     and l.action_code=x.item->>'recommended_action_code'
  ), adjusted as (
    select
      p.*,
      case
  ...
  with latest_outcome as (
    select distinct on (e.organization_id,e.workspace_id)
      e.organization_id,e.workspace_id,e.action_code,e.outcome_status,e.effectiveness_score,e.confidence,e.assessed_at
    from private.coach_action_outcome_events_v96 e
    where e.actor_id=p_actor_id
      and e.evidence_status='SUFFICIENT'
      and e.outcome_status<>'WAITING_EVIDENCE'
  ...
  ) order by l.organization_id,l.sample_size desc,l.action_code),'[]'::jsonb)
  into v_learning
  from learning l;
...
begin
  ...
  select w.organization_id,w.client_id,w.effective_action_code,w.recommendation_snapshot,w.completed_at,
         coalesce(nullif(w.recommendation_snapshot->>'primary_domain',''),upper(coalesce(w.execution_target,'')))
    into v_organization,v_client_id,v_action_code,v_baseline,v_completed_at,v_domain
  from private.coach_action_workspace_v95 w
  ...
  where x->>'organization_id'=v_organization::text
    and x->>'client_id'=v_client_id::text
  limit 1;
...
begin
  ...
  select w.organization_id,w.client_id into v_organization,v_client_id
  from private.coach_action_workspace_v95 w
  ...
  from private.coach_action_outcome_events_v96 e
  where e.organization_id=v_organization
    and e.workspace_id=p_workspace_id
    and e.evidence_fingerprint=v_assessment->>'evidence_fingerprint';

  if v_existing is null then
    insert into private.coach_action_outcome_events_v96(
      organization_id,workspace_id,actor_id,client_id,action_code,domain,outcome_status,evidence_status,
      effectiveness_score,confidence,evidence_fingerprint,assessment,next_check_after,assessed_at
    ) values(
      v_organization,p_workspace_id,p_actor_id,v_client_id,v_assessment->>'action_code',v_assessment->>'domain',v_assessment->>'outcome_status',v_assessment->>'evidence_status',
  ...
  if v_assessment->>'evidence_status'='SUFFICIENT' then
    update private.coach_action_workspace_v95
     ...
     where id=p_workspace_id and organization_id=v_organization and actor_id=p_actor_id and completion_verification is distinct from 'system_reconciled';
  end if;
...
begin
  ...
  with latest as (
    select distinct on (e.organization_id,e.workspace_id) e.*
    from private.coach_action_outcome_events_v96 e
    where e.actor_id=p_actor_id
    order by e.organization_id,e.workspace_id,e.assessed_at desc,e.id desc
  ), rows as (
    ...
    left join latest l on l.organization_id=w.organization_id and l.workspace_id=w.id
  ...
  with latest as (
    ...
    select organization_id,action_code,count(*)::int as sample_size,
    ...
    from sufficient group by organization_id,action_code
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    'organization_id',a.organization_id,
  ...
  ) order by a.organization_id,a.sample_size desc,a.action_code),'[]'::jsonb)
...
comment on function private.compute_coach_action_outcome_v96(uuid,uuid,timestamptz) is
  'F1.M1.S5 H3B2: action outcome evidence reads only the workspace Organization.';
comment on function public.reconcile_coach_action_outcome_v96(uuid,uuid) is
  'F1.M1.S5 H3B2: action outcome persistence is Organization-scoped.';
comment on function private.get_coach_outcome_intelligence_base_v96(uuid) is
  'F1.M1.S5 H3B2: historical outcome learning is partitioned by Organization.';
