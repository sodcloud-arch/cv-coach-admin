-- ARCH-1.0 · F1.M1.S5 G1B3B hotfix — AI preparation context tenant scope
-- Historical G1B3B migration is already live. This hotfix closes four residual
-- client-context reads that still keyed only by client_id.
...
comment on function public.prepare_ai_program_generation(uuid,uuid,uuid,text,integer,text) is
  'F1.M1.S5 G1B3B hotfix: onboarding, weekly check-in, training preferences and schedule preferences are bound to Program Organization.';
