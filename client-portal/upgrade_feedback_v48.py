...
async function cvCompleteWorkoutAfterFeedback(statusEl,button){
  try{
    ...
    clearInterval(timerHandle);cvCloseWorkoutFeedback({restoreRest:false});workout=null;await loadReal();view='home';render();cvShowWorkoutResult(r||{});
...
function cvOpenWorkoutFeedback(){
  ...
  document.body.appendChild(modal);cvFeedbackRestGuardV80(true);
