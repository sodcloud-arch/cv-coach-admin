...
new_wrappers = """  let nightSyncQueued=false;
  ...
  window.openDay=async function(id){
    ...
    if(mode==='real'&&user?.id){try{const q=await sb.from('workout_sessions').select('id,program_day_id,started_at,status').eq('client_id',user.id).eq('organization_id',activeOrganizationId).eq('status','in_progress').order('started_at',{ascending:false}).limit(1).maybeSingle();if(q.error)throw q.error;if(epoch!==sessionEpoch)return false;open=q.data||null;if(open?.program_day_id){effective=open.program_day_id;if(effective!==id)toast?.('Ya tenías una sesión activa · la recuperé')}}catch(e){console.warn('CV active session lookup',e)}}
...
old_v25_open = "  const oldOpen=window.openDay;window.openDay=async function(id){try{if(mode==='real'&&user?.id){const q=await sb.from('workout_sessions').select('program_day_id,started_at').eq('client_id',user.id).eq('organization_id',activeOrganizationId).eq('status','in_progress').order('started_at',{ascending:false}).limit(1).maybeSingle();if(q.data?.program_day_id&&q.data.program_day_id!==id){id=q.data.program_day_id;toast?.('Ya tenías una sesión activa · la recuperé')}}}catch(e){console.warn('v25 active workout',e)}return oldOpen.call(this,id)};"
