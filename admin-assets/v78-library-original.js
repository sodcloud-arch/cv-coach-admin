  ...
  window.library = async function libraryV78(){
    ...
    try{
      ...
      const draw=()=>{
        ...
        const filtered=rows.filter(r=>{const x=r.x, hay=`${x.name||''} ${x.slug||''} ${x.primary_muscle||''} ${x.equipment||''} ${x.movement_pattern||''}`.toLowerCase();const stateMatch=s==='all'||r.state===s;return (!q||hay.includes(q))&&(!m||x.primary_muscle===m)&&(!e||x.equipment===e)&&stateMatch});
