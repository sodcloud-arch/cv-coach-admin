...
(() => {
  ...
  function ensureStyle(){
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      .v94-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-end;flex-wrap:wrap;margin-bottom:14px}.v94-title{font-size:clamp(32px,5vw,54px);letter-spacing:-.04em;line-height:.95;margin:5px 0 8px}.v94-sub{max-width:880px;color:var(--m);line-height:1.5}
      .v94-kpis{display:grid;grid-template-columns:repeat(6,minmax(105px,1fr));gap:9px;margin:15px 0}.v94-kpi{background:#0d1013;border:1px solid var(--b);border-radius:12px;padding:12px}.v94-kpi b{display:block;font-size:26px}.v94-kpi span{font-size:9px;color:var(--m);text-transform:uppercase;letter-spacing:.07em}
      ...
      @media(max-width:1150px){.v94-kpis{grid-template-columns:repeat(3,1fr)}.v94-grid{grid-template-columns:1fr}}@media(max-width:720px){.v94-kpis{grid-template-columns:repeat(2,1fr)}.v94-toolbar{grid-template-columns:1fr}.v94-metrics{grid-template-columns:repeat(2,1fr)}.v94-actions .btn{flex:1}}
  ...
  function card(x){
    const [plabel,pcls]=pm(clean(x.priority));const [dlabel,dcls]=dm(clean(x.primary_domain));const t=x.training||{},l=x.lifecycle||{};
    ...
    return `<article class="v94-card" data-priority="${esc(x.priority||'NORMAL')}" data-client="${esc(x.client_id||'')}">
      <div class="row"><div class="grow"><div class="ey">V94 · COACH AI</div><h2 style="margin:4px 0 2px">${esc(x.client_name||'Cliente')}</h2><div class="muted">${esc(x.primary_goal||'Objetivo no definido')}</div></div><div class="v94-score">${esc(x.command_score??0)}<small>SCORE</small></div></div>
      <div class="v94-pills">${pill(plabel,pcls)}${pill(dlabel,dcls)}${pill('RIESGO '+clean(t.risk_level||'GREEN'),clean(t.risk_level)==='RED'?'v94-red':clean(t.risk_level)==='YELLOW'?'v94-warn':'v94-green')}${l.stage&&l.stage!=='operational'?pill(clean(l.stage).replaceAll('_',' ').toUpperCase(),'v94-gray'):''}</div>
  ...
  async function renderCommand(){
    ...
    try{
      ...
      root.innerHTML=`<div class="v94-head"><div><div class="ey">V94 · UNIFIED COMMAND CENTER</div><h1 class="v94-title">COACH AI</h1><p class="v94-sub">Prioriza qué cliente necesita atención, explica la evidencia y recomienda la siguiente acción. Combina V86 + V93 sin reemplazar sus motores y sin aplicar decisiones automáticamente.</p></div><button id="v94Refresh" class="btn">↻ ACTUALIZAR</button></div>
        <div class="v94-kpis"><div class="v94-kpi"><span>Clientes</span><b>${esc(s.total_clients??0)}</b></div><div class="v94-kpi"><span>Requieren atención</span><b>${esc(s.requires_attention??0)}</b></div><div class="v94-kpi"><span>Críticos</span><b>${esc(s.critical??0)}</b></div><div class="v94-kpi"><span>Alta prioridad</span><b>${esc(s.high??0)}</b></div><div class="v94-kpi"><span>Programación</span><b>${esc(s.programming??0)}</b></div><div class="v94-kpi"><span>Pilotos</span><b>${esc(s.active_pilots??0)}</b></div></div>
        ...
        <section><div class="row"><h2 class="grow">Cola priorizada</h2><span class="muted">score determinístico + evidencia</span></div><div id="v94Grid" class="v94-grid"></div></section>
        ...
        <div class="card" style="margin-top:20px"><div class="ey">GUARDRAILS V94</div><div class="muted" style="margin-top:7px">Solo recomendación · agregado read-only · auto-publicación OFF · auto-edición OFF · identidad sintética NO · revisión del coach obligatoria.</div></div>`;
    ...
    }catch(e){root.innerHTML=`<div class="card" style="border-color:#64202b"><b>No se pudo cargar Coach AI V94</b><div class="muted">${esc(String(e?.message||e))}</div></div>`;}
  }
  ...
  if(typeof render!=='function'||typeof setView!=='function'||typeof req!=='function'){console.error('CV Coach V94: admin host contract unavailable');return;}
  ...
  installNavigation();
  window.coachAICommandCenterV94={render:renderCommand,version:'V94'};
