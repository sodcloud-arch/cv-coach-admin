...
const expected={
  ...
  progression_contract:'CV_V84_ADAPTIVE_PROGRAMMING_E2E_OK',
  draft_contract:'CV_V84_ADAPTIVE_DRAFT_E2E_OK',
...
const draftExpected={
  ok:true,
  adaptation_state:'deload_recommended',
...
for(const [key,value] of Object.entries(draftExpected)){
  if(body?.draft?.[key]!==value)throw new Error(`V84 draft contract mismatch ${key}: expected ${JSON.stringify(value)}, got ${JSON.stringify(body?.draft?.[key])}`);
}
console.log('CV_V84_ADAPTIVE_PROGRAMMING_FULL_E2E_OK',JSON.stringify({run_id:RUN_ID,target:body.suggested_duration_seconds,draft_mode:body.draft.draft_mode}));
